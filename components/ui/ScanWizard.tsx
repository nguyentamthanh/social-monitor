...
import { Drawer, Steps, Checkbox, Button, Empty, Progress, message, Tag } from 'antd'
import {
  YoutubeFilled,
  GoogleSquareFilled,
  FacebookFilled,
...
import { BrandAsset, ConnectorStatus, Platform } from '@/types'
...
export default function ScanWizard({ open, onClose, onCompleted }: Props) {
  ...
  const startScan = async () => {
    if (selectedAssets.length === 0 || selectedPlatforms.length === 0) {
      message.warning('Hãy chọn ít nhất 1 tài sản và 1 nền tảng')
      return
    }
    setScanning(true)
      ...
      footer={
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {step === 0 && (
              <Button type="primary" onClick={() => setStep(1)} disabled={selectedAssets.length === 0}>
                Tiếp tục
              </Button>
            )}
            {step === 1 && (
        ...
        <div style={{ textAlign: 'center', padding: '32px 16px' }}>
          {scanProgress.status === 'running' && (
            <>
              ...
              <p style={{ color: '#a1a1aa', marginTop: 8 }}>
                Đang quét {selectedAssets.length} tài sản trên {selectedPlatforms.length} nền tảng...
              </p>
            </>
          )}
          {scanProgress.status === 'completed' && (
            <>
              ...
              <div style={{ margin: '16px 0' }}>
                <RiskPill score={scanProgress.findings * 5} size="lg" />
              </div>
              <p style={{ color: '#a1a1aa' }}>
                Phát hiện <strong style={{ color: '#fafafa' }}>{scanProgress.findings}</strong> kết quả.
                {scanId && (
                  <>
                    {' '}
                    <Link href="/findings" style={{ color: '#8b5cf6' }}>Xem chi tiết →</Link>
                  </>
                )}
              </p>
            </>
          ...
          {scanProgress.status === 'failed' && (
            <>
              ...
              <p style={{ color: '#a1a1aa' }}>Vui lòng kiểm tra cấu hình connector và thử lại.</p>
            </>
          )}
        </div>
      )}
